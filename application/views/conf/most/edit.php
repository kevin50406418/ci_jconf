...
<div class="ui raised segment">
	<div class="modal-header">
		...
		<a href="<?php echo get_url("dashboard",$conf_id,"most","detail")?>?id=<?php echo $most->most_id?>" class="ui button blue pull-right">查看</a>
		<?php echo form_open(get_url("dashboard",$conf_id,"most","edit")."?do=submit&id=".$most->most_id);?>
	...
	<?php echo form_open(get_url("dashboard",$conf_id,"most","edit")."?do=info&id=".$most->most_id,array("class"=>"form-horizontal"));?>
		<h2>計畫資料</h2>
		...
		<table class="table table-bordered">
			...
			<tr>
				...
				<td class="col-sm-10">
					...
					<label class="checkbox-inline"><input name="report_mealtype" type="radio" value="<?php echo $day?>"<?php if( $report->report_mealtype == $day ){?> checked<?php }?>> <?php echo $day?></label>
					<?php }?>
					<label class="checkbox-inline"><input name="report_mealtype" type="radio" value="S"<?php if( $report->report_mealtype == "S" ){?> checked<?php }?>>自理</label>
					<label class="checkbox-inline"><input name="report_mealtype" type="radio" value="P"<?php if( $report->report_mealtype == "P" ){?> checked<?php }?>>成果發表當天(隨議程而定)</label>
				</td>
			</tr>
		</table>
		...
		<table class="table table-bordered" id="file">
			<tr>
				...
				<td class="col-sm-10">
					...
					<div class="col-sm-8">
						<?php echo form_open_multipart(get_url("dashboard",$conf_id,"most","edit")."?do=file&id=".$most->most_id,array("class"=>"form-horizontal"));?>
							<div class="row">
								<div class="col-sm-10">
