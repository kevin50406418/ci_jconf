        jQuery(document).ready(function () {
            var options = {
                onLoad: function () {
                    //$('#alert-pwd').text('Start typing password');
                },
                onKeyUp: function (evt) {
                    $(evt.target).pwstrength("outputErrorList");
                }
            };
            $('#user_pw').pwstrength(options);
        });
...
(function ($) {
    ...
    var options = {
            ...
            minChar: 6,
            errorMessages: {
                password_to_short: "密碼長度過短，密碼必須是至少 6 字符。",
                same_as_username: "密碼不能包含帳號。"
            },
            scores: [17, 26, 40, 50],
            verdicts: ["很虛弱", "弱", "中", "讚", "極讚"],
            showVerdicts: false,
            raisePower: 1.4,
        ...
        setProgressBar = function ($el, score) {
            ...
            if (score < options.scores[0]) {
                ...
                progressbar.find(".progress-bar").css("width", "5%").text(options.verdicts[0]);
                if (options.showVerdicts) {
                    $verdict.text(options.verdicts[0]);
                }
            } else if (score >= options.scores[0] && score < options.scores[1]) {
                progressbar.addClass("progress-danger").removeClass("progress-warning").removeClass("progress-success");
                progressbar.find(".progress-bar").css("width", "25%").text(options.verdicts[1]);
                if (options.showVerdicts) {
                ...
                progressbar.find(".progress-bar").css("width", "50%").text(options.verdicts[2]);
                if (options.showVerdicts) {
                    $verdict.text(options.verdicts[2]);
                }
            } else if (score >= options.scores[2] && score < options.scores[3]) {
                progressbar.addClass("progress-warning").removeClass("progress-danger").removeClass("progress-success");
                progressbar.find(".progress-bar").css("width", "75%").text(options.verdicts[3]);
                if (options.showVerdicts) {
                ...
                progressbar.find(".progress-bar").css("width", "100%").text(options.verdicts[4]);
                if (options.showVerdicts) {
        ...
        methods = {
            ...
            destroy: function () {
                this.each(function (idx, el) {
                    ...
                    $el.parent().find("div.error-list").remove();
            ...
            outputErrorList: function () {
                this.each(function (idx, el) {
                    var output = '<div class="error-list alert alert-danger">',
                        $el = $(el),
